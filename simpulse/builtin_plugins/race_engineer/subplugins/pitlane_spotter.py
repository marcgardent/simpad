"""
SimPulse Race Engineer — Pitlane Spotter & Unsafe Release Protection Role (High Precision FSM).
        ...
        for opp in pit_opponents:
            ...
            if 0.0 < dist_behind <= self.unsafe_release_distance_m or (0.0 < euc_dist <= self.unsafe_release_distance_m and dist_behind >= -2.0):
                ...
                if dist_effective <= self.unsafe_release_distance_m or ttc <= self.unsafe_release_ttc_sec:
                    threats.append({
                        "id": opp.id,
                        "name": opp.driver_name or "Opponent",
                        "speed_mps": opp_speed,
                        "dist": dist_effective,
                        "ttc": ttc,
                        "opp": opp,
    ...
    def _handle_pitlane_driving_traffic(
        ...
        if self.enable_pit_traffic_ahead:
            ...
            for opp in pit_opponents:
                ...
                pit_state = opp.pit_state
                in_stall = opp.in_garage_stall
        ...
        if self.enable_pit_overlap:
            ...
            if alongside_cars:
                alongside_name = alongside_cars[0].driver_name or "Car"
