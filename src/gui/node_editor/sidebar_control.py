...
class NodeSidebarControl:
    ...
    def build_sidebar(self, parent_editor: Any):
        ...
        with dpg.child_window(width=320, height=-1, border=True):
            ...
            with dpg.group(horizontal=True):
                dpg.add_slider_float(label="L", tag="test_sensor_abs_l", default_value=0.0, min_value=0.0, max_value=1.0, format="%.2f", width=120, callback=lambda: self.on_test_sensor_change(parent_editor._synth_engine))
                dpg.add_slider_float(label="R", tag="test_sensor_abs_r", default_value=0.0, min_value=0.0, max_value=1.0, format="%.2f", width=120, callback=lambda: self.on_test_sensor_change(parent_editor._synth_engine))

            dpg.add_spacer(height=2)
            dpg.add_text("Over-Acceleration", color=[255, 220, 0, 255])
            with dpg.group(horizontal=True):
                dpg.add_slider_float(label="L", tag="test_sensor_tc_l", default_value=0.0, min_value=0.0, max_value=1.0, format="%.2f", width=120, callback=lambda: self.on_test_sensor_change(parent_editor._synth_engine))
                dpg.add_slider_float(label="R", tag="test_sensor_tc_r", default_value=0.0, min_value=0.0, max_value=1.0, format="%.2f", width=120, callback=lambda: self.on_test_sensor_change(parent_editor._synth_engine))

            dpg.add_spacer(height=2)
            dpg.add_text("Oversteer", color=[255, 220, 0, 255])
            with dpg.group(horizontal=True):
                dpg.add_slider_float(label="L", tag="test_sensor_over_l", default_value=0.0, min_value=0.0, max_value=1.0, format="%.2f", width=120, callback=lambda: self.on_test_sensor_change(parent_editor._synth_engine))
                dpg.add_slider_float(label="R", tag="test_sensor_over_r", default_value=0.0, min_value=0.0, max_value=1.0, format="%.2f", width=120, callback=lambda: self.on_test_sensor_change(parent_editor._synth_engine))
            ...
            with dpg.group(horizontal=True):
                dpg.add_button(label="Braking L", width=65, callback=lambda: self.trigger_sensor_pulse(parent_editor._synth_engine, "test_sensor_abs_l"))
                dpg.add_button(label="Braking R", width=65, callback=lambda: self.trigger_sensor_pulse(parent_editor._synth_engine, "test_sensor_abs_r"))
                dpg.add_button(label="Accel L", width=65, callback=lambda: self.trigger_sensor_pulse(parent_editor._synth_engine, "test_sensor_tc_l"))
                dpg.add_button(label="Accel R", width=65, callback=lambda: self.trigger_sensor_pulse(parent_editor._synth_engine, "test_sensor_tc_r"))
            with dpg.group(horizontal=True):
                dpg.add_button(label="Oversteer L", width=65, callback=lambda: self.trigger_sensor_pulse(parent_editor._synth_engine, "test_sensor_over_l"))
            ...
            dpg.add_button(label="Reset All Sensors to 0", width=280, callback=lambda: self.reset_test_sensors(parent_editor._synth_engine))
    ...
    def reset_test_sensors(self, synth_engine: Any):
        """Resets all live test sensor sliders to zero."""
        for tag in [
