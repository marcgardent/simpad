...
class NodeEditorTab:
    ...
    def _build_node_toolbox(self):
        ...
        with dpg.child_window(width=195, height=-1, border=True):
            ...
            dpg.add_spacer(height=2)
            dpg.add_button(label="+ Gear Sensor", width=-1, callback=lambda: self._add_node_sensor_gear())
            dpg.add_spacer(height=2)
            ...
            dpg.add_spacer(height=2)
            dpg.add_button(label="+ Invert (1 - x)", width=-1, callback=lambda: self._add_node_invert())
            dpg.add_spacer(height=2)
            ...
            dpg.add_spacer(height=2)
            dpg.add_button(label="+ Boolean Logic", width=-1, callback=lambda: self._add_node_boolean())
