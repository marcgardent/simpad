"""
monitoringBoard — Dedicated HUD Monitoring Overlay Dashboard.
Positioned in the Top-Middle third of the screen (X=Width/3, Y=0, W=Width/3, H=Height/3).
"""
...
class MonitoringBoard(BaseDashboard):
    """
    Tableau de bord 'monitoringBoard' positionné dans le tiers haut (vertical)
    et milieu (horizontal) de l'écran (grille 3x3 : col=1, row=0).
    """
    ...
    def update_telemetry(self, sensors: VehicleSensors) -> None:
        ...
        # Mise à jour des valeurs de télémétrie sur le tableau de bord
        ...
        if dpg.does_item_exist("mb_bar_rpm"):
            dpg.set_value("mb_bar_rpm", min(1.0, max(0.0, sensors.rpm_ratio)))
            dpg.configure_item("mb_bar_rpm", overlay=f"{int(sensors.rpm_ratio * sensors.engine_max_rpm)} RPM")
